...
export default function UserProfile() {
  ...
  return (
    <div className="animate-in fade-in duration-500 max-w-4xl mx-auto">
      ...
      {/* Header Profile Card */}
      <div className="bg-white rounded-[32px] md:rounded-[40px] shadow-sm border border-slate-100 p-8 md:p-12 mb-8">
        <div className="flex flex-col md:flex-row items-center gap-8 md:gap-10">
          <div
            className={`w-24 h-24 md:w-32 md:h-32 rounded-[32px] md:rounded-[40px] flex items-center justify-center overflow-hidden shadow-xl border-4 border-white ${isTeacher ? "bg-indigo-50 text-indigo-600" : "bg-emerald-50 text-emerald-600"}`}
          >
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-col md:flex-row items-center gap-3 mb-2">
              ...
              <span
                className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${isTeacher ? "bg-indigo-100 text-indigo-600" : "bg-emerald-100 text-emerald-600"}`}
              >
                {profileUser.role}
              </span>
            </div>
            <div className="flex flex-wrap justify-center md:justify-start gap-4 text-slate-400 font-bold text-xs mb-6 uppercase tracking-widest">
              <span className="flex items-center gap-1.5">
                <Icon name="Mail" className="w-4 h-4" /> {profileUser.email}
              </span>
              <span className="flex items-center gap-1.5">
            ...
            <div className="flex items-center justify-center md:justify-start gap-4">
              {isTeacher && currentUser?.role === "student" && (
                <button
                  ...
                  className={`px-10 py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all ${
                    isConnected ? "bg-emerald-50 text-emerald-600 cursor-default" : isPending ? "bg-slate-100 text-slate-400 cursor-default" : "bg-indigo-600 text-white shadow-xl shadow-indigo-100 hover:scale-[1.02]"
                  }`}
                >
              ...
              {currentUser?._id === profileUser._id && (
                <button
                  ...
                  className="flex items-center gap-2 bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100"
                >
